...
if (isset($_POST['btnUpdate'])) {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        $field_errors['csrf'] = 'Your session expired. Please refresh and try again.';
    }
    ...
    if (!$field_errors) {
        ...
        if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] !== UPLOAD_ERR_NO_FILE) {
            $up = nh_store_upload($_FILES['profile_image'], nh_avatar_dir(), 'image', 'user_' . $id);
            if ($up['ok']) {
                $new_profile = $up['filename'];
            } else {
                $upload_err = $up['error'];
            }
        }
        ...
        if ($upload_err) {
            ...
            $upd = mysqli_prepare($con, "UPDATE user_info SET username = ?, email = ?, phone = ?, profile = ? WHERE id = ?");
            mysqli_stmt_bind_param($upd, "ssssi", $name, $email, $phone, $new_profile, $id);
            if (mysqli_stmt_execute($upd)) {
                $flash = 'User profile updated successfully.';
                mysqli_stmt_close($upd);

                $q = mysqli_prepare($con, "SELECT * FROM user_info WHERE id = ? LIMIT 1");
                mysqli_stmt_bind_param($q, "i", $id);
                mysqli_stmt_execute($q);
                $fresh = mysqli_fetch_assoc(mysqli_stmt_get_result($q));
...
<style>
    ...
    .uu-input:focus { border-color: #3b82f6; box-shadow: 0 0 0 3px rgba(59,130,246,.12); }
    ...
    .uu-upload:hover, .uu-upload.drag { border-color: #3b82f6; background: rgba(59,130,246,.06); transform: translateY(-2px); }
    .uu-upload i { font-size: 1.6rem; color: #3b82f6; opacity: .75; }
    ...
    .uu-btn-save {
        ...
        box-shadow: 0 8px 18px -8px rgba(59,130,246,.6);
    }
    .uu-btn-save:hover { color: #fff; box-shadow: 0 12px 24px -8px rgba(59,130,246,.8); }
    .uu-btn-cancel {
    ...
    .uu-toast.success .ic { background: linear-gradient(135deg,#059669,#059669); }
    .uu-toast.error .ic { background: linear-gradient(135deg,#dc2626,#dc2626); }
    ...
    <div class="container" style="margin-top: -34px;">
        ...
        <div class="row">
            <div class="col-lg-4 mb-3 uu-reveal uu-d1">
                <div class="uu-card" style="height:100%;">
                    <div class="uu-card-head">
                        <h5><span class="uu-ico" style="background:rgba(26,86,219,.1);color:var(--primary);"><i class="fas fa-id-badge"></i></span>Profile</h5>
                    </div>
                    <div class="p-4 text-center">
                        ...
                        <div class="mt-3 d-flex flex-column align-items-start" style="gap:6px;text-align:left;">
                            <?php if ($result['user_degree'] !== ''): ?>
                                <span class="badge" style="background:rgba(6,182,212,.12);color:#0ea5e9;padding:6px 12px;font-size:.75rem;font-weight:700;">
                                    <i class="fas fa-graduation-cap mr-1"></i><?php echo htmlspecialchars($result['user_degree']); ?>
                                </span>
                            <?php endif; ?>
                            <?php if ($result['user_skills'] !== ''): ?>
                                <span class="badge" style="background:rgba(5,150,105,.12);color:#059669;padding:6px 12px;font-size:.75rem;font-weight:700;">
                                    <i class="fas fa-code mr-1"></i><?php echo htmlspecialchars($result['user_skills']); ?>
                                </span>
                            <?php endif; ?>
                            <?php if ($result['experience'] !== ''): ?>
                                <span class="badge" style="background:rgba(217,119,6,.14);color:#b45309;padding:6px 12px;font-size:.75rem;font-weight:700;">
            ...
            <div class="col-lg-8 uu-reveal uu-d2">
                <div class="uu-card">
                    <div class="uu-card-head">
                        <h5><span class="uu-ico" style="background:rgba(5,150,105,.1);color:#059669;"><i class="fas fa-sliders-h"></i></span>Account Details</h5>

...
-- ------------------------------------------------------------------
-- NO DEFAULT ADMIN IS SHIPPED (security).
-- A well-known default credential is one of the most common ways a
-- deployed site gets taken over, so the admin account is NOT seeded here.
--
-- Create your admin account once, from the command line:
--
--   php -r "echo password_hash('YOUR_STRONG_PASSWORD', PASSWORD_BCRYPT), PHP_EOL;"
--
-- then paste the resulting hash into this statement and run it ONCE:
--
--   INSERT INTO `admin_login` (`admin_user_name`, `admin_password`)
--   VALUES ('your_admin_name', 'PASTE_THE_BCRYPT_HASH_HERE');
--
-- (On XAMPP, php is at /Applications/XAMPP/xamppfiles/bin/php)
-- ------------------------------------------------------------------

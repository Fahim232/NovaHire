...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_message'])) {
    ...
    $msg_id = send_message($con, 'user', $user_id, $receiver_type, $receiver_id, $subject, $message, $related_job_id);
...
if ($active_conversation) {
    mark_conversation_read($con, 'user', $user_id, $active_conversation['type'], $active_conversation['id']);
}
...
$conversations_q = mysqli_query($con, "
    SELECT m.*,
        ...
        CASE 
            WHEN m.sender_type = 'company' THEN (SELECT logo FROM companies WHERE id = m.sender_id)
            ELSE NULL
        END as sender_logo,
        (SELECT COUNT(*) FROM messages WHERE sender_type = m.sender_type AND sender_id = m.sender_id AND receiver_type = m.receiver_type AND receiver_id = m.receiver_id AND is_read = 0 AND receiver_type = 'user' AND receiver_id = $user_id) as unread_count
    FROM messages m
    WHERE (m.sender_type = 'user' AND m.sender_id = $user_id AND m.is_deleted_by_sender = 0)
       OR (m.receiver_type = 'user' AND m.receiver_id = $user_id AND m.is_deleted_by_receiver = 0)
    GROUP BY 
        CASE 
            WHEN m.sender_type = 'user' THEN CONCAT(m.receiver_type, '_', m.receiver_id)
...
while ($row = mysqli_fetch_assoc($conversations_q)) {
    if ($row['sender_type'] === 'user' && $row['sender_id'] == $user_id) {
...
$messages = [];
if ($active_conversation) {
    $messages = get_conversation($con, 'user', $user_id, $active_conversation['type'], $active_conversation['id'], 50);
    
    // Get the name of the other party
    if ($active_conversation['type'] === 'company') {
        $other_q = mysqli_query($con, "SELECT company_name, logo FROM companies WHERE id = " . $active_conversation['id']);
        $other_info = mysqli_fetch_assoc($other_q);
        $active_name = $other_info['company_name'] ?? 'Company';
        $active_logo = $other_info['logo'] ?? '';
    } else {
        $other_q = mysqli_query($con, "SELECT username, profile FROM user_info WHERE id = " . $active_conversation['id']);
        $other_info = mysqli_fetch_assoc($other_q);
        $active_name = $other_info['username'] ?? 'User';
        $active_logo = $other_info['profile'] ?? '';
    }
}
?>
<!DOCTYPE html>
            ...
            <?php if ($active_conversation && !empty($active_name)): ?>
                ...
                <div class="mc-chat-messages" id="chatMessages">
                    <?php foreach ($messages as $msg):
                        $is_sent = ($msg['sender_type'] === 'user' && $msg['sender_id'] == $user_id);
    ...
    <div class="compose-modal">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                <label class="font-weight-bold small">Send To (Company)</label>
                <select name="receiver_id" class="form-control" required id="newMsgReceiver" style="border-radius: 10px;">
                    <option value="">Select a company...</option>
                    <?php
                    $companies_q = mysqli_query($con, "SELECT id, company_name FROM companies WHERE status='active' ORDER BY company_name");
                    while ($comp = mysqli_fetch_assoc($companies_q)):
                    ?>

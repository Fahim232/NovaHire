...
if (isset($_POST['register']) && !isset($error)) {
    ...
    if ($email_exists) {
        ...
        if (!isset($error)) {
            ...
            mysqli_stmt_bind_param($ins_stmt, "sssssssssss", $company_name, $email, $phone, $address, $website, $industry, $company_size, $description, $logo_name, $hashed_password, $status = 'active');
...
<head>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .cr-input-wrap input,.cr-input-wrap textarea,.cr-input-wrap select{width:100%;padding:13px 14px 13px 42px;border:2px solid var(--cr-border);border-radius:var(--cr-radius);font-size:.9rem;color:var(--cr-text);font-weight:500;font-family:inherit;transition:all .3s;background:var(--cr-bg);outline:none}
    ...
    @media(max-width:991px){.cr-split{grid-template-columns:1fr}.cr-brand{display:none}}
    @media(max-width:575px){.cr-form-panel{padding:24px 16px}.cr-step-label{display:none}.cr-step-line{width:24px}.cr-grid-2{grid-template-columns:1fr}.cr-btn-row{flex-direction:column-reverse}.cr-btn-back{flex:1}.cr-brand{display:none}}
    </style>
</head>
<body>
    <div class="cr-split">
        <div class="cr-brand">
            <div class="cr-brand-content">
                <div class="cr-brand-logo"><div class="cr-brand-logo-icon"><i class="fas fa-layer-group"></i></div><div class="cr-brand-logo-text">Nova<span>Hire</span></div></div>
                ...
                <div class="cr-features">
                    <div class="cr-feature"><div class="cr-feature-icon"><i class="fas fa-bolt"></i></div><div class="cr-feature-text"><h6>Post Jobs in Minutes</h6><small>Reach thousands of qualified candidates instantly</small></div></div>
                    <div class="cr-feature"><div class="cr-feature-icon"><i class="fas fa-robot"></i></div><div class="cr-feature-text"><h6>AI-Powered Matching</h6><small>Smart candidate recommendations for every role</small></div></div>
                    <div class="cr-feature"><div class="cr-feature-icon"><i class="fas fa-chart-line"></i></div><div class="cr-feature-text"><h6>Hiring Analytics</h6><small>Track performance and optimize your pipeline</small></div></div>
                </div>
                <div class="cr-stats">
            ...
            <div class="cr-brand-footer"><i class="fas fa-shield-halved"></i> Trusted by leading companies across Bangladesh</div>
        </div>
        <div class="cr-form-panel">
            <div class="cr-form-wrap">
                <div class="cr-form-header"><h2>Create Employer Account</h2><p>Fill in the details to get started</p></div>
                <div class="cr-steps">
                    <div class="cr-step active" id="crStepInd1"><span class="cr-step-num">1</span><span class="cr-step-label">Company</span></div>
                    <div class="cr-step-line" id="crLine1"></div>
                    <div class="cr-step" id="crStepInd2"><span class="cr-step-num">2</span><span class="cr-step-label">Contact</span></div>
                    <div class="cr-step-line" id="crLine2"></div>
                    <div class="cr-step" id="crStepInd3"><span class="cr-step-num">3</span><span class="cr-step-label">Account</span></div>
                </div>
                <?php if (isset($error_msg)): ?>
                    <div class="cr-alert error"><i class="fas fa-exclamation-circle"></i><?= $error_msg ?></div>
                <?php endif; ?>
                <?php if (isset($success_msg)): ?>
                    <div class="cr-alert success"><i class="fas fa-check-circle"></i><?= $success_msg ?> Redirecting... <i class="fas fa-spinner fa-spin ml-2"></i></div>
                ...
                <form method="POST" action="" enctype="multipart/form-data" id="crForm">
                    ...
                    <div class="cr-form-step active" id="crStep1">
                        <div class="cr-field"><label>Company Name <span class="req">*</span></label><div class="cr-input-wrap"><input type="text" name="company_name" placeholder="e.g. Tech Solutions Inc." required maxlength="255" id="crCompanyName"><i class="fas fa-building"></i></div></div>
                        <div class="cr-field"><label>Company Logo <small>(optional)</small></label><label class="cr-logo-upload" for="crLogoInput" id="crLogoLabel"><div class="cr-logo-preview" id="crLogoPreview"><i class="fas fa-cloud-arrow-up"></i></div><div class="cr-logo-info"><h6>Click to upload logo</h6><small>JPG, PNG, GIF or WebP - Max 5MB</small></div></label><input type="file" name="logo" id="crLogoInput" accept="image/*" style="display:none"></div>
                        <div class="cr-field"><label>Company Description <span class="req">*</span></label><div class="cr-input-wrap"><textarea name="description" rows="3" placeholder="Tell us about your company, mission, and culture..." required id="crCompanyDesc"></textarea><i class="fas fa-info-circle" style="top:30px;transform:none"></i></div></div>
                        <div class="cr-btn-row"><button type="button" class="cr-btn cr-btn-next" onclick="crNext(2)">Continue <i class="fas fa-arrow-right"></i></button></div>
                    </div>
                    <div class="cr-form-step" id="crStep2">
                        <div class="cr-field"><label>Company Email <span class="req">*</span></label><div class="cr-input-wrap"><input type="email" name="email" placeholder="hr@company.com" required id="crEmail"><i class="fas fa-envelope"></i></div></div>
                        <div class="cr-field"><label>Phone Number <span class="req">*</span></label><div class="cr-input-wrap"><input type="tel" name="phone" placeholder="+880 1XXXXXXXXX" required id="crPhone"><i class="fas fa-phone"></i></div></div>
                        <div class="cr-field"><label>Website <small>(optional)</small></label><div class="cr-input-wrap"><input type="url" name="website" placeholder="https://example.com"><i class="fas fa-globe"></i></div></div>
                        <div class="cr-field"><label>Company Address <span class="req">*</span></label><div class="cr-input-wrap"><textarea name="address" rows="2" placeholder="Street, City, Country" required id="crAddress"></textarea><i class="fas fa-location-dot" style="top:30px;transform:none"></i></div></div>
                        <div class="cr-grid-2">
                            <div class="cr-field" style="margin:0"><label>Industry <span class="req">*</span></label><div class="cr-input-wrap"><select name="industry" required id="crIndustry"><option value="">Select Industry</option><option value="Information Technology">Information Technology</option><option value="Software Development">Software Development</option><option value="Web Development">Web Development</option><option value="Mobile App Development">Mobile App Development</option><option value="E-commerce">E-commerce</option><option value="Finance">Finance</option><option value="Healthcare">Healthcare</option><option value="Education">Education</option><option value="Marketing">Marketing</option><option value="Consulting">Consulting</option><option value="Manufacturing">Manufacturing</option><option value="Retail">Retail</option><option value="Media">Media & Entertainment</option><option value="Other">Other</option></select><i class="fas fa-industry"></i></div></div>
                            <div class="cr-field" style="margin:0"><label>Company Size <span class="req">*</span></label><div class="cr-input-wrap"><select name="company_size" required id="crSize"><option value="">Select Size</option><option value="1-10">1-10 employees</option><option value="11-50">11-50 employees</option><option value="51-200">51-200 employees</option><option value="201-500">201-500 employees</option><option value="501-1000">501-1000 employees</option><option value="1000+">1000+ employees</option></select><i class="fas fa-users"></i></div></div>
                        </div>
                        <div class="cr-btn-row"><button type="button" class="cr-btn cr-btn-back" onclick="crPrev(1)"><i class="fas fa-arrow-left"></i> Back</button><button type="button" class="cr-btn cr-btn-next" onclick="crNext(3)">Continue <i class="fas fa-arrow-right"></i></button></div>
                    </div>
                    <div class="cr-form-step" id="crStep3">
                        <div class="cr-field"><label>Password <span class="req">*</span></label><div class="cr-input-wrap"><input type="password" name="password" placeholder="Min. 8 characters" required minlength="8" id="crPw" oninput="crCheckStrength(this.value)"><i class="fas fa-lock"></i><span class="cr-pw-toggle" onclick="crTogglePw('crPw',this)"><i class="fas fa-eye"></i></span></div><div class="cr-pw-strength"><div class="cr-pw-strength-bar" id="crPwBar"></div></div><div class="cr-hint" id="crPwHint"></div></div>
                        <div class="cr-field"><label>Confirm Password <span class="req">*</span></label><div class="cr-input-wrap"><input type="password" name="cpassword" placeholder="Re-enter password" required minlength="8" id="crCpw" oninput="crMatchPw()"><i class="fas fa-lock"></i><span class="cr-pw-toggle" onclick="crTogglePw('crCpw',this)"><i class="fas fa-eye"></i></span></div><div class="cr-hint" id="crPwMatch"></div></div>
                        <div class="cr-terms" style="margin-top:12px"><input type="checkbox" name="terms" id="crTerms" required><label for="crTerms">I agree to the <a href="#">Terms of Service</a> and <a href="#">Privacy Policy</a>. I confirm that I am authorized to register this company.</label></div>
                        <div class="cr-btn-row"><button type="button" class="cr-btn cr-btn-back" onclick="crPrev(2)"><i class="fas fa-arrow-left"></i> Back</button><button type="submit" name="register" class="cr-btn cr-btn-submit" id="crSubmitBtn" disabled><i class="fas fa-check-circle"></i> Create Account</button></div>
                    </div>
                </form>
                <div class="cr-login-link">Already have an account? <a href="auth/login.php">Sign in here</a></div>
            </div>
        </div>
    </div>
    <script>
    let crCurrent=1;
    function crShow(step){document.querySelectorAll('.cr-form-step').forEach(s=>s.classList.remove('active'));document.getElementById('crStep'+step).classList.add('active');for(let i=1;i<=3;i++){const ind=document.getElementById('crStepInd'+i);ind.classList.remove('active','done');const line=document.getElementById('crLine'+(i-1));if(i<step){ind.classList.add('done');if(line)line.classList.add('done')}else if(i===step){ind.classList.add('active')}else{if(line)line.classList.remove('done')}}crCurrent=step;window.scrollTo({top:0,behavior:'smooth'})}
    function crValidate1(){const n=document.getElementById('crCompanyName').value.trim(),d=document.getElementById('crCompanyDesc').value.trim();if(!n||!d){crShake(['crCompanyName','crCompanyDesc']);return false}return true}
    function crValidate2(){const e=document.getElementById('crEmail').value.trim(),p=document.getElementById('crPhone').value.trim(),i=document.getElementById('crIndustry').value,s=document.getElementById('crSize').value,a=document.getElementById('crAddress').value.trim();if(!e||!p||!i||!s||!a){crShake(['crEmail','crPhone']);return false}return true}
    function crNext(step){if(step===2&&!crValidate1())return;if(step===3&&!crValidate2())return;crShow(step)}
    function crPrev(step){crShow(step)}
    function crShake(ids){ids.forEach(id=>{const el=document.getElementById(id);if(el&&!el.value.trim()){el.style.borderColor='#dc2626';el.style.animation='crShake .4s ease';setTimeout(()=>el.style.animation='',500)}})}
    document.getElementById('crLogoInput').addEventListener('change',function(e){const f=e.target.files[0];if(f){const r=new FileReader();r.onload=function(ev){document.getElementById('crLogoPreview').innerHTML='<img src="'+ev.target.result+'">';document.getElementById('crLogoLabel').classList.add('has-file')};r.readAsDataURL(f)}});
    function crTogglePw(id,toggle){const inp=document.getElementById(id),icon=toggle.querySelector('i');if(inp.type==='password'){inp.type='text';icon.classList.replace('fa-eye','fa-eye-slash')}else{inp.type='password';icon.classList.replace('fa-eye-slash','fa-eye')}}
    function crCheckStrength(pw){const bar=document.getElementById('crPwBar'),hint=document.getElementById('crPwHint');let score=0;if(pw.length>=6)score++;if(pw.length>=8)score++;if(/[A-Z]/.test(pw))score++;if(/[0-9]/.test(pw))score++;if(/[^A-Za-z0-9]/.test(pw))score++;const colors=['#dc2626','#f97316','#eab308','#22c55e','#059669'],labels=['Very Weak','Weak','Fair','Strong','Very Strong'],idx=Math.min(score,4);if(pw.length===0){bar.style.width='0';hint.textContent=''}else{bar.style.width=((idx+1)*20)+'%';bar.style.background=colors[idx];hint.textContent=labels[idx];hint.style.color=colors[idx]}crMatchPw();crCheckReady()}
    function crMatchPw(){const pw=document.getElementById('crPw').value,cpw=document.getElementById('crCpw').value,hint=document.getElementById('crPwMatch');if(cpw.length===0){hint.textContent='';return}if(pw===cpw){hint.innerHTML='<i class="fas fa-check-circle"></i> Passwords match';hint.style.color='#059669'}else{hint.innerHTML='<i class="fas fa-times-circle"></i> Passwords do not match';hint.style.color='#dc2626'}crCheckReady()}
    function crCheckReady(){const pw=document.getElementById('crPw').value,cpw=document.getElementById('crCpw').value,t=document.getElementById('crTerms').checked,btn=document.getElementById('crSubmitBtn');btn.disabled=!(pw.length>=8&&pw===cpw&&t)}
    document.getElementById('crTerms').addEventListener('change',crCheckReady);
    var s=document.createElement('style');s.textContent='@keyframes crShake{0%,100%{transform:translateX(0)}25%{transform:translateX(-6px)}75%{transform:translateX(6px)}}';document.head.appendChild(s);
    </script>
</body>
</html>

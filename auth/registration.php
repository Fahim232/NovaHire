...
<head>
    ...
    <style>
        body{min-height:100vh;display:flex;align-items:center;justify-content:center;overflow-y:auto;padding:40px 0}
        .reg-container{width:100%;max-width:1000px;background:rgba(255,255,255,.95);backdrop-filter:blur(20px);border-radius:30px;box-shadow:0 25px 50px rgba(0,0,0,.15);overflow:hidden;display:flex;position:relative;margin:20px}
        .reg-visual{width:40%;background:linear-gradient(135deg,#0984e3 0%,#00cec9 100%);display:flex;flex-direction:column;justify-content:center;align-items:center;padding:40px;text-align:center;color:#fff;position:relative}
        .reg-visual::before{content:'';position:absolute;width:400px;height:400px;background:rgba(255,255,255,.1);border-radius:50%;top:-100px;left:-100px}
        .reg-visual h2{font-size:2.5rem;font-weight:800;margin-bottom:20px;position:relative;z-index:2}
        .reg-visual p{font-size:1.1rem;opacity:.9;position:relative;z-index:2}
        .reg-btn-outline{border:2px solid #fff;color:#fff;background:transparent;padding:10px 30px;border-radius:50px;font-weight:700;margin-top:30px;transition:all .3s;position:relative;z-index:2}
        .reg-btn-outline:hover{background:#fff;color:#0984e3;transform:translateY(-3px);box-shadow:0 10px 20px rgba(0,0,0,.1)}
        .reg-form-side{width:60%;padding:50px;background:#fff;overflow-y:auto;max-height:90vh}
        ...
        .input-group-modern>i{position:absolute;left:20px;top:50%;transform:translateY(-50%);color:#b2bec3;transition:color .3s;z-index:5}
        .form-control-modern{width:100%;padding:15px 15px 15px 50px;border:2px solid #f1f2f6;border-radius:15px;font-size:.95rem;color:#2d3436;font-weight:500;transition:all .3s;background:#fdfdfd;height:auto}
        .form-control-modern:focus{border-color:#00cec9;background:#fff;outline:none;box-shadow:0 5px 20px rgba(0,206,201,.1)}
        .btn-modern{background:linear-gradient(to right,#0984e3,#00cec9);color:#fff;border:none;padding:15px;border-radius:15px;font-weight:700;letter-spacing:1px;text-transform:uppercase;font-size:.9rem;transition:transform .3s;width:100%;cursor:pointer;margin-top:10px}
        .btn-modern:hover{transform:translateY(-3px);box-shadow:0 10px 20px rgba(9,132,227,.3);color:#fff}
        .skills-section{margin-bottom:20px}
        .skills-label{font-size:.85rem;font-weight:700;color:#636e72;text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;display:flex;align-items:center;gap:8px}
        .skills-label i{color:#00cec9}
        .selected-skills-display{display:flex;flex-wrap:wrap;gap:6px;min-height:38px;padding:10px 14px;border:2px solid #f1f2f6;border-radius:12px;background:#f8f9fa;margin-bottom:10px;transition:all .3s}
        .selected-skills-display:focus-within{border-color:#00cec9;box-shadow:0 5px 20px rgba(0,206,201,.1)}
        .selected-skill-tag{display:inline-flex;align-items:center;gap:5px;background:linear-gradient(135deg,#0984e3,#00cec9);color:#fff;padding:4px 12px;border-radius:20px;font-size:.78rem;font-weight:600;animation:tagPop .2s ease}
        .selected-skill-tag .remove-skill{cursor:pointer;opacity:.8;font-size:.7rem;margin-left:2px}
        ...
        .no-skills-msg{color:#b2bec3;font-size:.85rem;font-style:italic}
        .skill-categories-wrap{border:2px solid #f1f2f6;border-radius:12px;overflow:hidden;max-height:0;transition:max-height .3s ease,opacity .3s ease;opacity:0}
        .skill-categories-wrap.open{max-height:500px;opacity:1;overflow-y:auto}
        .skill-cat-header{padding:10px 16px;font-size:.78rem;font-weight:800;text-transform:uppercase;letter-spacing:.8px;background:#f1f5f9;color:#475569;border-bottom:1px solid #e2e8f0;position:sticky;top:0;z-index:2}
        .skill-cat-header i{margin-right:6px}
        .skill-options{display:flex;flex-wrap:wrap;gap:6px;padding:10px 14px}
        .skill-option{display:inline-flex;align-items:center;gap:5px;padding:5px 14px;border-radius:20px;font-size:.8rem;font-weight:600;border:2px solid #e2e8f0;background:#fff;color:#475569;cursor:pointer;transition:all .2s;user-select:none}
        .skill-option:hover{border-color:#0984e3;color:#0984e3;background:#eff6ff}
        .skill-option.selected{background:linear-gradient(135deg,#0984e3,#00cec9);color:#fff;border-color:transparent}
        .skill-option input{display:none}
        .skills-toggle-btn{display:inline-flex;align-items:center;gap:6px;padding:8px 16px;border:2px dashed #00cec9;border-radius:10px;background:transparent;color:#00cec9;font-weight:700;font-size:.85rem;cursor:pointer;transition:all .2s;width:100%;justify-content:center}
        .skills-toggle-btn:hover{background:#f0fdfa;border-style:solid}
        .skills-toggle-btn i.toggle-icon{transition:transform .3s}
        .skills-toggle-btn.open i.toggle-icon{transform:rotate(180deg)}
        @media(max-width:991px){.reg-container{flex-direction:column;max-width:500px}.reg-visual{width:100%;height:200px;padding:20px}.reg-visual::before{display:none}.reg-form-side{width:100%;padding:30px;max-height:none}.reg-visual h2{font-size:1.8rem;margin-bottom:10px}.reg-btn-outline{margin-top:15px}}
        .reg-photo-section{text-align:center;margin-bottom:24px}
        .reg-photo-wrap{display:inline-block;position:relative;cursor:pointer}
        .reg-photo-preview{width:100px;height:100px;border-radius:50%;border:3px dashed #00cec9;background:#f0fdfa;display:flex;flex-direction:column;align-items:center;justify-content:center;transition:all .3s;overflow:hidden}
        .reg-photo-preview:hover{border-color:#0984e3;background:#eff6ff;transform:scale(1.05)}
        .reg-photo-preview i{font-size:1.5rem;color:#00cec9;margin-bottom:2px}
        .reg-photo-preview span{font-size:.7rem;font-weight:700;color:#636e72;text-transform:uppercase;letter-spacing:.3px}
        .reg-photo-preview img{width:100%;height:100%;object-fit:cover;border-radius:50%}
        .reg-photo-preview.has-photo i,.reg-photo-preview.has-photo span{display:none}
        .reg-photo-hint{font-size:.78rem;color:#b2bec3;margin-top:6px;font-weight:500}
    </style>
</head>
<body>
    <?php
            ...
            <form action="<?= htmlentities($_SERVER['PHP_SELF']) ?>" method="POST" enctype="multipart/form-data">
                ...
                <div class="reg-photo-section">
                    <div class="reg-photo-wrap" id="regPhotoWrap">
                        <div class="reg-photo-preview" id="regPhotoPreview"><i class="fas fa-camera"></i><span>Add Photo</span></div>
                ...
                <div class="input-group-modern"><input name="username" type="text" placeholder="Full Name" class="form-control-modern" required><i class="fas fa-user"></i></div>
                <div class="input-group-modern"><input name="email" type="email" placeholder="Email Address" class="form-control-modern" required><i class="fas fa-envelope"></i></div>
                <div class="row">
                    <div class="col-md-6"><div class="input-group-modern"><input name="phone" type="text" placeholder="Phone" class="form-control-modern" maxlength="10" required><i class="fas fa-phone"></i></div></div>
                    <div class="col-md-6"><div class="input-group-modern"><select name="degree" class="form-control-modern" required style="appearance:none;-webkit-appearance:none"><option value="" disabled selected>Select Degree</option><option value="BE/BTech">BE/BTech</option><option value="ME/MTech">ME/MTech</option><option value="BCA">BCA</option><option value="MCA">MCA</option><option value="BSc">BSc</option><option value="MSc">MSc</option><option value="Other">Other</option></select><i class="fas fa-graduation-cap"></i></div></div>
                </div>
                <div class="skills-section">
                    <div class="skills-label"><i class="fas fa-code"></i> Your Skills (at least 1 required)</div>
                    <div class="selected-skills-display" id="selectedSkillsDisplay"><span class="no-skills-msg" id="noSkillsMsg">Click below to add your skills</span></div>
                    <input type="hidden" name="user_skills" id="userSkillsInput" value="">
                    <button type="button" class="skills-toggle-btn" id="skillsToggle" onclick="toggleSkillPanel()"><i class="fas fa-plus-circle"></i> Choose Skills <i class="fas fa-chevron-down toggle-icon" id="toggleIcon"></i></button>
                ...
                <div class="row">
                    <div class="col-md-6"><div class="input-group-modern"><input name="password" type="password" placeholder="Password" class="form-control-modern" required><i class="fas fa-lock"></i></div></div>
                    <div class="col-md-6"><div class="input-group-modern"><input name="cpassword" type="password" placeholder="Confirm" class="form-control-modern" required><i class="fas fa-lock"></i></div></div>
                </div>
    ...
    <script>
    let selectedSkills=[];
    function toggleSkillPanel(){const p=document.getElementById('skillPanel'),b=document.getElementById('skillsToggle');p.classList.toggle('open');b.classList.toggle('open')}
    function toggleSkill(l){const c=l.querySelector('input'),s=c.value;if(c.checked){selectedSkills.push(s);l.classList.add('selected')}else{selectedSkills=selectedSkills.filter(sk=>sk!==s);l.classList.remove('selected')}updateSkillsDisplay()}
    function removeSkill(s){selectedSkills=selectedSkills.filter(sk=>sk!==s);document.querySelectorAll('.skill-option input').forEach(c=>{if(c.value===s){c.checked=false;c.closest('.skill-option').classList.remove('selected')}});updateSkillsDisplay()}
    function updateSkillsDisplay(){const d=document.getElementById('selectedSkillsDisplay'),i=document.getElementById('userSkillsInput');i.value=selectedSkills.join(', ');d.innerHTML=selectedSkills.length===0?'<span class="no-skills-msg">Click below to add your skills</span>':selectedSkills.map(s=>'<span class="selected-skill-tag">'+s+' <span class="remove-skill" onclick="removeSkill(\''+s+'\')"><i class="fas fa-times-circle"></i></span></span>').join('')}
    document.getElementById('submitBtn').addEventListener('click',function(e){if(selectedSkills.length===0){e.preventDefault();alert('Please select at least one skill.');toggleSkillPanel()}});
    </script>
    <script>
    var rw=document.getElementById('regPhotoWrap'),ri=document.getElementById('regPhotoInput'),rp=document.getElementById('regPhotoPreview');
    rw.addEventListener('click',function(){ri.click()});
    ri.addEventListener('change',function(e){var f=e.target.files[0];if(!f)return;var r=new FileReader();r.onload=function(ev){rp.innerHTML='<img src="'+ev.target.result+'" alt="Photo">';rp.classList.add('has-photo')};r.readAsDataURL(f)});
    </script>
</body>
</html>
